...
(function ($, Drupal, drupalSettings, CKEDITOR) {
  ...
  CKEDITOR.plugins.add('ccom_faq_linkit', {
    init: function (editor) {
      // Add the commands for link and unlink.
      editor.addCommand('ccom_faq_linkit', {
        ...
        exec: function (editor) {
          ...
          var saveCallback = function (returnValues) {
            ...
            if (!linkElement && returnValues.attributes.href) {
              ...
              if (range.collapsed) {
                ...
                console.log(returnValues.attributes);
                var text = new CKEDITOR.dom.text(returnValues.attributes.title, editor.document);
          ...
          Drupal.ckeditor.openDialog(editor, Drupal.url('faq_linkit/dialog/linkit/' + editor.config.drupal.format), existingValues, saveCallback, dialogSettings);
        }
      });

      // CTRL + L.
      editor.setKeystroke(CKEDITOR.CTRL + 76, 'ccom_faq_linkit');

      // Add buttons.
      if (editor.ui.addButton) {
        editor.ui.addButton('FaqLinkit', {
          label: Drupal.t('Link'),
          command: 'ccom_faq_linkit',
      ...
      editor.on('doubleclick', function (evt) {
        ...
        if (!element.isReadOnly()) {
          if (element.is('a')) {
            ...
            editor.getCommand('ccom_faq_linkit').exec();
          }
        }
      });
